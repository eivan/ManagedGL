...
// Kisebb módosításokat eszközöltem.
...
namespace ManagedGL.Textures
{
    ...
    public class TextRenderer : IDisposable
    {
        ...
        public void DrawSliceY(byte[, ,] map, int layer, PointF point)
        {
            for (int i = 0; i < map.GetLength(0); ++i)
                for (int j = 0; j < map.GetLength(2); ++j)
                {
                    byte v = map[i, layer, j];
                    bmp.SetPixel((int)point.X + i, (int)point.Y + j,
                        v > 128 ? Color.Transparent : Color.FromArgb(v, v, v));// < 128 ? Color.Transparent : Color.LawnGreen
                }
            Size size = new Size(map.GetLength(0), map.GetLength(2));
